...
def packList(l):
    while len(l) > 1:
        ...
        if first[0] <= second[0] and first[1] >= second[1]:
            #print(first)
            l.append(first)
        elif first[0] <= second[0] <= first[1]+1  or first[0] <= second[0] <= first[1]:
            #print((first[0], second[1]))
            l.append((first[0], second[1]))
        elif first[0] >= second[0] and first[1] <= second[1]:
            #print(second)
            l.append(second)
        elif first[0]+1 >= second[0] >= first[1] or second[0] <= first[0] <= second[1]:
            #print((second[0], first[1]))
            l.append((second[0], first[1]))
        else:
            #print(5)
            l.append(first)
            l.append(second)
...
# max = 20 # for example
max = 4000000 # for test

for y in range(2680000, max+1):
    if y%10000==0:
        print(y)
    impossibleAtY = []
    for sensor in sensors:
        impossibleAtY.append(sensor.impossibleBeaconsAt(y))
    impossibleAtY = [item for item in impossibleAtY if item is not None]
    packList(impossibleAtY)
    if len(impossibleAtY) > 1:
        print(y, impossibleAtY)
